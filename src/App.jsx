...
import { Target, Activity, LayoutDashboard, ClipboardList, CheckCircle2, XCircle, Clock } from 'lucide-react';
  ...
  if (decisions.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-24 text-center">
        ...
        <p className="text-xs text-slate-300 mt-1">Accept or reject an agent recommendation in Scenario Planning to see it here.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-base font-bold text-ink">Signal Decision Log</h2>
          <p className="text-xs text-muted mt-0.5">{decisions.length} decision{decisions.length !== 1 ? 's' : ''} recorded this session</p>
        </div>
                ...
                return (
                  ...
                  >
                    ...
                    {t.id === 'log' && decisions.length > 0 && (
                      <span className="ml-0.5 px-1.5 py-0.5 text-[10px] font-bold rounded-full bg-teal-100 text-teal-700">{decisions.length}</span>
                    )}
                  </button>
                );
              })}
